...
export const createSite = async (siteData: any, orgId: string | null, userId?: string) => {
  ...
  try {
    ...
    debug.error('Error in createSite', error);
    throw error;
  }
};
...
export const getSiteById = async (id: string /*, orgId?: string | null */) => {
  debug.data('Fetching site by ID', { id });
  try {
    const { data, error } = await supabase
      .from('sites')
      .select('*')
      ...
      .single();
    if (error) {
      debug.error('Failed to fetch site by ID', error);
      throw error;
    }
    debug.data('Successfully fetched site', data);
    return data;
  } catch (error) {
    debug.error('Error in getSiteById', error);
    throw error;
  }
};

export const updateSite = async (id: string, updates: any /*, orgId?: string | null */) => {
  debug.data('Updating site by ID', { id, updates });
  try {
    const { data, error } = await supabase
      .from('sites')
      .update(updates)
      ...
      .select();
    if (error) {
      debug.error('Failed to update site', error);
      throw error;
    }
    debug.data('Successfully updated site', data);
    return data;
  } catch (error) {
    debug.error('Error in updateSite', error);
    throw error;
  }
}; 