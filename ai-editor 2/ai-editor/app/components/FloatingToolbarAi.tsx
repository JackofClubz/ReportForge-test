...
function extractUserInputs(text: string): Record<string, string[]> {
  ...
  // Naive regex example: find QP names (you can improve this)
  const qpMatches = text.match(/(?:qualified persons?|qps?)[:\-]?\s*([A-Z][a-z]+(?:\s+[A-Z][a-z]+)+)/gi);
  if (qpMatches) {
    userInputs["Qualified Person"] = qpMatches.map(name => name.replace(/qualified persons?:?\s*/i, "").trim());
  }

  // Add similar rules for other placeholders if needed
  // e.g., Company, Location, Commodity, etc.
...
export function FloatingToolbarAi({
  ...
  async (prompt: string) => {
    ...
    if (prompt === "Expand using RAG") {
      try {
        const section = detectSection(textContent ?? "");
        const userInputs = extractUserInputs(textContent ?? "");
        ...
        setMessages([
          ...messages,
          { content: textContent || "", role: "system" },
          { content: "Expand using RAG", role: "user" },
          { content: data.answer, role: "assistant" },
        ]);
        setAiState("complete");
      } catch (err) {
        setMessages([
          ...messages,
          { content: "RAG expansion failed.", role: "assistant" },
        ]);
        setAiState("complete");
      }
      return;
    ...
    // Default fallback for standard prompts
              ...
              {aiState === "initial" ? (
                ...
                <>
                  {page ? (
                    ...
                    <>
                      {/* Always show RAG at the top */}
                      <CommandItem
                        icon={<SparklesIcon className="h-full" />} // You can swap for RagIcon if available
                        onSelect={async () => {
                          setAiState("loading");
                          setInput("");
                          setPreviousPrompt("Expand using RAG");
                          try {
                            const res = await fetch("/api/rag-query", {
                              method: "POST",
                              headers: { "Content-Type": "application/json" },
                              body: JSON.stringify({ query: textContent }),
                            });
                            const data = await res.json();
                            setMessages([
                              ...messages,
